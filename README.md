...
冪等キーの別ボディ再利用 / `409`=使用中 race の削除 or 種族名の重複 or 冪等キーが処理中 **or
creation_token が消費済み（論理削除済み行がトークンを保持し再生不可）**—クライアントは新しい
`Idempotency-Key` で再試行する必要がある / `412`=楽観ロックの版不一致 /
`429`=レート制限（`RATE_LIMIT_RPS` 有効時） /
